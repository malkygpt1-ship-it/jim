...
function pickImage(html,base){
  ...
  for(const re of metas){const m=html.match(re);if(m){const u=absolute(base,m[1]);if(u)return u}}
  ...
  for(const m of html.matchAll(/<img\b[^>]*>/gi)){
    ...
    for(const a of ['data-zoom-image','data-large-image','data-src','data-original','src']){
      ...
      const u=absolute(base,raw);if(!u)continue;
      let score=0;
      if(/product|zoom|large|gallery|main/i.test(text+' '+u))score+=8;
      if(/\.(?:jpe?g|png|webp)(?:\?|$)/i.test(u))score+=4;
      if(/logo|icon|sprite|badge|payment|brand/i.test(text+' '+u))score-=10;
