...
function snapFive(value,fallback=0){
  const n=Number(value);
  const safe=Number.isFinite(n)?n:fallback;
  return Math.min(100,Math.max(0,Math.round(safe/5)*5));
}
function setPercentSelect(id,value,fallback=0){
  const el=$('#'+id);if(!el)return;
  el.value=String(snapFive(value,fallback));
}
function markupPct(){return Math.min(100,Math.max(0,+$('#materialMarkup')?.value||0))}
function toolMarkupPct(){return Math.min(100,Math.max(0,+$('#toolMarkup')?.value||0))}
...
function toolQuoteUnit(r){return (+r.cost||0)*(1+toolMarkupPct()/100)}
...
function renderRows(kind){
  ...
  body.innerHTML=state[kind].map((r,i)=>{
    ...
    const qty=+r.qty||0,cost=+r.cost||0,quote=toolQuoteUnit(r);
    return `<tr><td class="suggest-cell"><input class="row-input desc" autocomplete="off" data-kind="tools" data-i="${i}" data-f="name" value="${esc(r.name||'')}" placeholder="Click or type to search..."><div class="suggestions hidden"></div></td><td><input class="row-input qty" type="number" min="0" step="0.01" data-kind="tools" data-i="${i}" data-f="qty" value="${r.qty??1}"></td><td class="num tool-cost-col"><span class="tool-unit-cost">${money(cost)}</span></td><td class="num tool-cost-col"><strong class="tool-line-cost">${money(qty*cost)}</strong></td><td class="num tool-quote-col"><div class="derived-price tool-derived-price" title="Cost ${money(cost)} + ${toolMarkupPct()}% markup">${money(quote)}</div></td><td class="num tool-quote-col"><strong class="tool-quote-total">${money(qty*quote)}</strong></td><td><button class="remove" data-remove="tools" data-i="${i}">×</button></td></tr>`;
  }).join('');
}
function addRow(kind,row){
  const next=row||(kind==='materials'?{name:'',qty:1,cost:0}:{name:'',qty:1,cost:0});
...
function showSuggestions(input){
  ...
  box.innerHTML=matches.length?matches.map(x=>`<button type="button" class="suggestion" data-pick="${kind}" data-i="${input.dataset.i}" data-key="${esc(x.id||x.name)}"><span>${esc(x.name)}</span><strong>Cost ${money(x.cost)}</strong></button>`).join(''):'<div class="suggest-empty">No matching items</div>';
...
function totals(){
  ...
  const toolCost=state.tools.reduce((a,r)=>a+(+r.qty||0)*(+r.cost||0),0);
  const tool=state.tools.reduce((a,r)=>a+(+r.qty||0)*toolQuoteUnit(r),0);
  const labour=(+$('#hours').value||0)*(+$('#labourRate').value||0);
  const discount=Math.min(100,Math.max(0,+$('#discount').value||0))/100;
  return{materialCost,tradeSaving,billMat,mat,toolCost,tool,labour,total:(mat+tool+labour)*(1-discount)};
}
function recalc(){
  const t=totals();
  ...
  [...toolBody.querySelectorAll('tr')].forEach((tr,i)=>{
    const r=state.tools[i];if(!r)return;
    const qty=+r.qty||0,cost=+r.cost||0,quote=toolQuoteUnit(r);
    const unitCost=tr.querySelector('.tool-unit-cost');if(unitCost)unitCost.textContent=money(cost);
    const lineCost=tr.querySelector('.tool-line-cost');if(lineCost)lineCost.textContent=money(qty*cost);
    const quoteUnit=tr.querySelector('.tool-derived-price');if(quoteUnit){quoteUnit.textContent=money(quote);quoteUnit.title=`Cost ${money(cost)} + ${toolMarkupPct()}% markup`}
    const quoteTotal=tr.querySelector('.tool-quote-total');if(quoteTotal)quoteTotal.textContent=money(qty*quote);
  });
...
function snapshot(){return{reference:$('#reference').value,customer:$('#customer').value,phone:$('#phone').value,date:$('#date').value,address:$('#address').value,work:$('#work').value,hours:$('#hours').value,labourRate:$('#labourRate').value,materialMarkup:$('#materialMarkup').value,toolMarkup:$('#toolMarkup').value,tradeDiscount:$('#tradeDiscount').value,discount:$('#discount').value,materials:state.materials,tools:state.tools,updated:new Date().toISOString(),total:totals().total}}
function loadSnap(s){
  for(const k of ['reference','customer','phone','date','address','work','hours','discount'])if(s[k]!=null)$('#'+k).value=s[k];
  setPercentSelect('tradeDiscount',s.tradeDiscount,0);
  setPercentSelect('materialMarkup',s.materialMarkup,DEFAULT_MARKUP);
  setPercentSelect('toolMarkup',s.toolMarkup,DEFAULT_TOOL_MARKUP);
  $('#labourRate').value=s.labourRate??s.labourSell??20;
  state.materials=structuredClone(s.materials||[]);
  state.tools=structuredClone(s.tools||[]).map(r=>({name:r.name||'',qty:r.qty??1,cost:+r.cost||0,catalogId:r.catalogId||r.id}));
...
document.addEventListener('input',e=>{
  ...
  if(el.dataset.kind){
    ...
    if(el.dataset.f==='name'){
      ...
      if(hit){r.catalogId=hit.id;r.cost=hit.cost;if(el.dataset.kind==='materials')delete r.quoteOverride}
    }
  }
  recalc();saveDraft();
});
document.addEventListener('mousedown',e=>{
  ...
  if(r&&hit){
    Object.assign(r,{catalogId:hit.id,name:hit.name,cost:hit.cost});
...
$('#toolMarkup').addEventListener('change',()=>{recalc();saveDraft()});
$('#tradeDiscount').addEventListener('change',()=>{recalc();saveDraft()});
...
$('#newBtn').onclick=()=>{if(confirm('Start a new estimate?')){const n=parseInt(($('#reference').value.match(/\d+/)||['3072'])[0],10)+1;loadSnap({reference:`GF ${n}`,customer:'',phone:'',date:new Date().toISOString().slice(0,10),address:'',work:'',hours:0,labourRate:20,materialMarkup:$('#materialMarkup').value,toolMarkup:$('#toolMarkup').value,tradeDiscount:$('#tradeDiscount').value,discount:0,materials:[],tools:[]})}};
...
async function init(){
  ...
  if(draft)loadSnap(draft);else{setPercentSelect('materialMarkup',DEFAULT_MARKUP,DEFAULT_MARKUP);setPercentSelect('toolMarkup',DEFAULT_TOOL_MARKUP,DEFAULT_TOOL_MARKUP);setPercentSelect('tradeDiscount',0,0);addRow('materials');addRow('tools',{name:'',qty:0,cost:0});recalc()}
}
init();
